...
/-!
# Finite counting for the scaled exponent lattice

The main estimate is proved without measure theory.  Dividing coordinate `i`
by `i + 1` embeds an ordinary discrete simplex into the scaled simplex times
a box of remainders.  Multiplying in the opposite direction embeds that
product into a slightly enlarged ordinary simplex.  Stars and bars then gives
the two polynomial bounds.
-/
...
theorem mem_scaledExponentBox {d z : ℕ} {c : ScaledExponent d} :
  ...
  simp only [scaledExponentBox, Fintype.mem_piFinset, Finset.mem_range]
  exact forall_congr' fun _ ↦ Nat.lt_succ_iff
...
theorem mem_scaledExponentFinset {d z : ℕ} {c : ScaledExponent d} :
  ...
  rw [scaledExponentFinset, Finset.mem_filter, mem_scaledExponentBox]
  constructor
  · exact And.right
  · intro hc
    exact ⟨fun i ↦ (scaledExponent_coordinate_le_weight c i).trans hc, hc⟩
...
theorem goodScaledExponentCount_eq_of_le {d z S : ℕ} (hzS : z ≤ S) :
    goodScaledExponentCount d z S = scaledExponentCount d z := by
  simp [goodScaledExponentCount, scaledExponentCount,
...
theorem scaledExponentEquivHigherJet_weight {d : ℕ}
  ...
  simp [scaledWeight, scaledExponentEquivHigherJet, smul_eq_mul, mul_comm]
...
theorem scaledExponentEquivHigherJet_degree {d : ℕ}
  ...
  simp [higherJetDegree, scaledOrdinaryDegree, Finsupp.degree_eq_sum,
    scaledExponentEquivHigherJet]
...
def scaledWithResidueToOrdinary (d z : ℕ)
  ...
  ⟨fun i ↦ (Nat.divModEquiv (i.val + 1)).symm (p.1.1 i, p.2 i), by
    ...
    · simpa [scaledWeight, Nat.mul_comm] using
