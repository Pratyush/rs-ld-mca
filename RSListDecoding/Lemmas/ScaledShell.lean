...
`interpolationWeightBudget` after substituting `d = derivativeOrder ε θ`. -/
...
theorem scaledShellWeight_derivativeOrder (ε θ : ℝ) :
    scaledShellWeight θ (derivativeOrder ε θ) =
      interpolationWeightBudget ε θ := by
  simp [scaledShellWeight, interpolationWeightBudget, multiplicity]

@[simp]
theorem scaledShellDegree_derivativeOrder (ε θ : ℝ) :
    scaledShellDegree θ (derivativeOrder ε θ) =
...
/-- Rounded-parameter form of the shell theorem.  The returned threshold
depends only on `θ`; any positive `ε` whose derivative order is beyond it
satisfies both explicit hypotheses, the cardinality comparison, and the
claimed real upper estimate for the natural ceiling factor. -/
theorem exists_scaledShellThreshold_for_roundedParameters
    {θ : ℝ} (hθ : 0 < θ) (hθ₁ : θ < 1) :
    ∃ d₀ : ℕ, ∀ ε : ℝ, 0 < ε →
      d₀ ≤ derivativeOrder ε θ →
      2 * ((derivativeOrder ε θ - 1) *
          (interpolationWeightBudget ε θ -
              (higherJetDegreeBudget ε θ + 1) +
            (derivativeOrder ε θ - 1)) ^
              (derivativeOrder ε θ - 1)) ≤
          interpolationWeightBudget ε θ ^
            (derivativeOrder ε θ - 1) ∧
      2 * (interpolationWeightBudget ε θ + multiplicity ε θ +
          derivativeOrder ε θ * (derivativeOrder ε θ - 1) / 2) ^
            (derivativeOrder ε θ - 1) ≤
      ...
      (scaledShellFactor θ (derivativeOrder ε θ) : ℝ) ≤
        2 * (derivativeOrder ε θ : ℝ) ^ scaledShellExponent θ := by
  obtain ⟨d₀, hd₀⟩ := exists_scaledShellThreshold hθ hθ₁
  refine ⟨d₀, fun ε hε hd ↦ ?_⟩
  let d := derivativeOrder ε θ
  have hp := hd₀ d hd
  have hbad := hp.1
  have hratio := hp.2
  have hcount := scaledShell_cardinality_bound hbad hratio
  have hfactor := scaledShellFactor_cast_le_two_rpow hθ hθ₁
    (show 1 ≤ d from derivativeOrder_pos θ hε)
  dsimp [d] at hbad hratio hcount hfactor
  simpa [multiplicity] using And.intro hbad <|
    And.intro hratio <| And.intro hcount hfactor
